...
`lifeRecords[]` 和全部 principle 的 `quotes[]`/`cases[]` 按日期合并成一条统一时间线——
纯生活记录一个颜色，链接到思维库的条目按所属象限上色（复用 `--quad-*` 变量）+ 一个跳转
chip。这是运行时合并（`src/template.html` 的 `collectTimelineItems()`），不是新数据结构，
改思维库或生活记录任一处，时间线自动跟着变。2017-09-25 到 2021 年直接用现有数据，2022 年
起要更详细（回头逐年重新翻读原始日记 PDF 做真正的逐日覆盖，是持续进行的内容工作，不是
一次性做完），完整规则见 `UPDATE.md` 的"「时间线记录」Tab"一节。
