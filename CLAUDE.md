...
思维库/原话集/时间线记录 属于日记，普世智金字塔/读书思维库/关联图谱/书籍索引 属于读书）。

**"时间线记录"（原"生活记录"，2026-07-26 改造）**：不再只是琐碎生活流水账，而是从
`meta.timelineStart`（`2017-09-25`，用户指定的纪念日，之前的日记不纳入）开始，把
`lifeRecords[]`、`healthLog[]` 和全部 principle 的 `quotes[]`/`cases[]` 按日期合并成一条
统一时间线——生活记录/健康追踪/四象限各一个颜色（健康追踪用专属的 `--health-accent`，
不再单独置顶一个"全家健康追踪"区块，而是混排进时间线、靠颜色和顶部图例区分），链接到
思维库的条目按所属象限上色（复用 `--quad-*` 变量）+ 一个跳转 chip。顶部有个可点击的颜色
图例条，点一个分类就筛选时间线只看这一类（比如只看生活记录回顾流水账），再点一次恢复全部。
这是运行时合并（`src/template.html` 的 `collectTimelineItems()`），不是新数据结构，改思维库/
生活记录/健康记录任一处，时间线自动跟着变。2017-09-25 到 2021 年直接用现有数据，2022 年
起要更详细（回头逐年重新翻读原始日记 PDF 做真正的逐日覆盖，是持续进行的内容工作，不是
一次性做完）。**健康类内容默认进 `healthLog[]`，不进思维库**——`MAS-C-20`（身体维护协议）
这类健康相关的思维库条目只留少数有代表性的案例，避免被大量同质化的"生病—反思—教训"流水账
稀释，完整细节在时间线的"健康追踪"分类里看。完整规则见 `UPDATE.md` 的"「时间线记录」Tab"
一节。

**读书宇宙内部两层，不要混为一谈**：
- **普世智金字塔**（`universalTruths[]`）是"每日强化考试"——42 天 Mind Card 学习法逐天产出，
  每条都是跨书综合、带场景审讯的深度条目，数量少（目前 3 条），慢慢长。
- **读书思维库**（`library[]`）是"百科全书"——一次性通读全部 40 本书笔记蒸馏出的全量索引
  （目前 262 条，覆盖全部 40 本书），参照 Mind OS 思维库的卡片做法（keyword/title/core +
